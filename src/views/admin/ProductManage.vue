<template>
  <div>
    <n-space justify="space-between" style="margin-bottom: 16px">
      <n-input v-model:value="keyword" placeholder="搜索产品名称" clearable style="width: 300px" @update:value="handleSearch">
        <template #prefix>
    ...
    <n-modal v-model:show="showModal" :title="editingId ? '编辑产品' : '新增产品'" preset="dialog" positive-text="确认" negative-text="取消" @positive-click="handleSubmit" style="width: 600px">
      <n-form :model="formData" label-placement="left" label-width="100">
        <n-form-item label="产品名称">
          <n-input v-model:value="formData.name" />
        </n-form-item>
        <n-form-item label="安装包位置">
          <n-input v-model:value="formData.install_path" />
        </n-form-item>
        <n-form-item label="安装手册位置">
          <n-input v-model:value="formData.install_manual_path" />
        </n-form-item>
        <n-form-item label="使用手册位置">
          <n-input v-model:value="formData.usage_manual_path" />
        </n-form-item>
        <n-form-item label="验收参数位置">
          <n-input v-model:value="formData.verification_path" />
        </n-form-item>
      </n-form>
    </n-modal>
  </div>
...
import { NInput, NButton, NDataTable, NModal, NForm, NFormItem, NSpace, NIcon, useMessage, useDialog } from 'naive-ui'
...
const columns: DataTableColumns = [
  { title: '产品名称', key: 'name', ellipsis: { tooltip: true }, width: 200 },
...
async function fetchData() {
  loading.value = true
  try {
    const res: any = await request.get('/products', { params: { page: pagination.page, pageSize: pagination.pageSize, keyword: keyword.value } })
...
function openModal(row?: any) {
  if (row) {
    ...
    Object.assign(formData, { name: row.name, install_path: row.install_path, install_manual_path: row.install_manual_path, usage_manual_path: row.usage_manual_path, verification_path: row.verification_path })
  } else {
    editingId.value = null
    Object.assign(formData, { name: '', install_path: '', install_manual_path: '', usage_manual_path: '', verification_path: '' })
  }
  showModal.value = true
}

async function handleSubmit() {
  if (!formData.name) { message.warning('请输入产品名称'); return false }
  if (editingId.value) {
